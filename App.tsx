...
const App: React.FC = () => {
  ...
  const StyleSwitcher = () => (
    <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] bg-white/80 backdrop-blur-xl border border-white p-2 rounded-full shadow-2xl flex items-center space-x-2 transition-all duration-500 ${activeServiceId ? 'translate-y-24 opacity-0 pointer-events-none' : ''}`}>
      <button 
        onClick={() => setStyle(DesignStyle.MODERN_CORPORATE)}
        className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${style === DesignStyle.MODERN_CORPORATE ? 'bg-brand-green text-white' : 'text-slate-600 hover:bg-slate-100'}`}
      >
        Modern
      </button>
      <button 
        onClick={() => setStyle(DesignStyle.GOURMET_DARK)}
        className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${style === DesignStyle.GOURMET_DARK ? 'bg-brand-green text-white' : 'text-slate-600 hover:bg-slate-100'}`}
      >
        Gourmet
      </button>
      <button 
        onClick={() => setStyle(DesignStyle.FRESH_ORGANIC)}
        className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${style === DesignStyle.FRESH_ORGANIC ? 'bg-brand-green text-white' : 'text-slate-600 hover:bg-slate-100'}`}
      >
        Organic
      </button>
    </div>
  ...
  const selectedService = SERVICES_DATA.find(s => s.id === activeServiceId);

  return (
    <div className="relative min-h-screen">
      <Header isDark={style === DesignStyle.GOURMET_DARK && !activeServiceId} />
      ...
      <div className="fixed bottom-8 right-8 z-[100] group">
         ...
         <button className="w-14 h-14 bg-brand-green rounded-full shadow-2xl flex items-center justify-center hover:bg-brand-dark transition-all active:scale-90 animate-bounce">
