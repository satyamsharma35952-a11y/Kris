...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, icon, variant, bgImage, delay }) => {
  const baseClasses = `relative p-8 rounded-[2.5rem] h-[400px] flex flex-col justify-center items-center text-center transition-all duration-500 ease-out hover:-translate-y-4 hover:shadow-[0_30px_60px_-15px_rgba(6,78,59,0.3)] group overflow-hidden opacity-0 animate-fade-in-up`;
  
  const variants = {
    dark: 'bg-[#064e3b] text-white',
    image: 'bg-slate-900 text-white',
    light: 'bg-white text-slate-900 border border-slate-100 shadow-sm hover:border-green-100'
  };
    ...
    <div className={`${baseClasses} ${variants[variant]} ${delay}`} style={{ animationFillMode: 'forwards' }}>
      {variant === 'image' && (
        <>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#064e3b]/90 to-transparent"></div>
        </>
      )}

      <div className="relative z-10 flex flex-col items-center">
        {/* Icon with float animation */}
        <div className={`mb-6 transform transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 ${variant === 'light' ? 'text-green-600' : 'text-green-400'}`}>
          {icon}
        </div>

        <h3 className="text-xl lg:text-2xl font-black mb-4 tracking-tight uppercase">{title}</h3>
        
        <p className={`text-sm leading-relaxed mb-8 max-w-[240px] ${variant === 'light' ? 'text-slate-500' : 'text-green-100/70'}`}>
          {description}
        </p>

        <a href="#" className={`inline-flex items-center space-x-2 font-bold text-sm transition-all ${variant === 'light' ? 'text-green-600' : 'text-green-400'} hover:space-x-4`}>
      ...
      {/* Decorative inner glow for dark cards */}
      {variant !== 'light' && (
...
export const WhatWeDo: React.FC = () => {
  return (
    <section className="py-32 bg-[#F7FAF7] relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <div className="text-center mb-20 space-y-6">
          <div className="inline-flex items-center space-x-4 bg-white px-6 py-2 rounded-full border border-slate-100 shadow-sm">
             ...
             <span className="text-green-700 font-bold text-xs uppercase tracking-[0.3em]">What We Do</span>
             <div className="h-[1px] w-8 bg-green-600"></div>
          </div>
          <h2 className="text-5xl lg:text-7xl font-bold text-slate-900 tracking-tight leading-tight max-w-4xl mx-auto">
            We Have Services For <span className="relative inline-block italic font-serif text-green-600">
              Catering
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <ServiceCard 
            ...
            description="Reliable catering tailored for industries in Noida and Gurgaon, ensuring workforce satisfaction."
          ...
          <ServiceCard 
            ...
            description="Freshly prepared culinary wonders served directly at your venue with professional flair."
          ...
          <ServiceCard 
            ...
            description="Nutritious and hygienic meal programs designed specifically for healthcare environments."
          ...
          <ServiceCard 
            ...
            description="Light, crisp, and nutrient-dense options for the health-conscious professional."
          ...
          <ServiceCard 
            ...
            description="A global palette of flavors from traditional Indian to contemporary continental."
